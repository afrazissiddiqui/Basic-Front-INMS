...
import { ApInvoiceDialogComponent } from './ap-invoice-dialog.component';
  ...
  template: `
    <div class="header-container animate-fade-in">
      <div class="title-section">
        ...
        <p class="subtitle">Manage and track your vendor invoices and purchases</p>
      </div>
      <button mat-flat-button color="primary" class="add-btn" (click)="openAddInvoiceDialog()">
        <mat-icon>add_shopping_cart</mat-icon>
        <span>Create AP Invoice</span>
      </button>
    </div>
    ...
    <mat-card class="table-card shadow-premium animate-fade-in">
      <div class="table-toolbar">
        <div class="search-wrapper">
          ...
          <input type="text" placeholder="Search by invoice # or vendor..." (keyup)="applyFilter($event)" class="creative-search">
        </div>
          ...
          <ng-container matColumnDef="Vendor">
            ...
            <td mat-cell *matCellDef="let element"> 
              <div class="vendor-info">
                <span class="vendor-name">{{element.BusinessPartnerName || 'Unknown Vendor'}}</span>
      ...
      <div class="no-data" *ngIf="!isLoading && !errorMessage && dataSource.length === 0">
        <div class="empty-state">
          ...
          <h3>No AP invoices found</h3>
          <p>Post your first purchase invoice to start tracking payables.</p>
        </div>
      </div>
    </mat-card>
  `,
  styles: [`
    ...
    .hover-row {
      ...
      &:hover {
        ...
        .id-cell { color: var(--accent-electric); }
      }
    }
...
export class ApInvoiceComponent implements OnInit {
  displayedColumns: string[] = ['InvoiceNumber', 'InvoiceDate', 'Vendor', 'LinesCount', 'actions'];
  dataSource: any[] = [];
  ...
  loadInvoices() {
    ...
    this.invoiceService.getInvoices().subscribe({
      ...
      error: (err) => {
        console.error(err);
        this.errorMessage = 'Failed to load invoices. Ensure backend is running.';
        this.isLoading = false;
      }
    });
  }
  ...
  openAddInvoiceDialog() {
    this.router.navigate(['/invoices/ap/new']);
  }

  viewInvoice(invoice: any) {
    this.router.navigate(['/invoices/ap/view', invoice.Id]);
  }

  editInvoice(invoice: any) {
    this.router.navigate(['/invoices/ap/edit', invoice.Id]);
  }
