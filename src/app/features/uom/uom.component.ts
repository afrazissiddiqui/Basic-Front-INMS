...
@Component({
  ...
  template: `
    <div class="header-container animate-fade-in">
      <div class="title-section">
        ...
        <p class="subtitle">Define and manage standard units for your inventory</p>
      </div>
      <button mat-flat-button color="primary" class="add-btn" (click)="openAddUomDialog()">
    ...
    <mat-card class="table-card shadow-premium animate-fade-in">
      <div class="table-toolbar">
        <div class="search-wrapper">
          ...
          <input type="text" placeholder="Search UOMs..." (keyup)="applyFilter($event)" class="creative-search">
        </div>
      ...
      <div class="error-message" *ngIf="errorMessage">
        ...
        <button mat-stroked-button color="primary" (click)="loadUoms()">Try Again</button>
      </div>
        ...
        <table mat-table [dataSource]="filteredDataSource" class="mat-elevation-z0">
          ...
          <ng-container matColumnDef="Name">
            ...
            <td mat-cell *matCellDef="let element"> 
              <div class="uom-info">
                <span class="uom-name">{{element.Name}}</span>
                <span class="uom-sub">Standard Measure</span>
              </div>
            </td>
          </ng-container>
          ...
          <ng-container matColumnDef="Abbreviation">
            ...
            <td mat-cell *matCellDef="let element"> 
              <span class="abbr-badge">{{element.Abbreviation}}</span>
            </td>
          </ng-container>
          ...
          <ng-container matColumnDef="actions">
            ...
            <td mat-cell *matCellDef="let element">
              <div class="action-group">
                <button mat-icon-button class="action-btn view" matTooltip="View UOM" (click)="viewUom(element)">
                  <mat-icon>visibility</mat-icon>
                </button>
                <button mat-icon-button class="action-btn edit" matTooltip="Edit UOM" (click)="editUom(element)">
                  <mat-icon>edit_note</mat-icon>
                </button>
                <button mat-icon-button class="action-btn delete" matTooltip="Delete UOM" (click)="deleteUom(element)">
      ...
      <div class="no-data" *ngIf="!isLoading && !errorMessage && dataSource.length === 0">
        <div class="empty-state">
          ...
          <p>Define your first unit of measure to start tracking inventory accurately.</p>
        </div>
      </div>
    </mat-card>
  `,
  styles: [`
    ...
    .uom-info {
      ...
      flex-direction: column;
      gap: 4px;
      .uom-name { font-weight: 700; color: var(--primary-deep); font-size: 16px; }
      .uom-sub { font-size: 11px; color: #94a3b8; font-weight: 600; text-transform: uppercase; }
    }
    ...
    .hover-row {
      ...
      &:hover {
        ...
        .uom-name { color: var(--accent-electric); }
      }
    }
...
export class UomComponent implements OnInit {
  displayedColumns: string[] = ['Id', 'Name', 'Abbreviation', 'actions'];
  dataSource: UnitOfMeasure[] = [];
  filteredDataSource: UnitOfMeasure[] = [];
  isLoading = true;
  errorMessage = '';
  ...
  loadUoms() {
    ...
    this.uomService.getUoms().subscribe({
      ...
      error: (err) => {
        ...
        this.errorMessage = 'Failed to load Unit of Measure list. Please check backend connection.';
        this.isLoading = false;
      }
    });
  }

  viewUom(uom: UnitOfMeasure) {
    this.dialog.open(UomDialogComponent, {
      width: '450px',
      data: { uom, mode: 'view' }
    });
  }
  ...
  deleteUom(uom: UnitOfMeasure) {
    Swal.fire({
      title: 'Are you sure?',
      text: `Do you really want to delete "${uom.Name}"? This unit may be in use by some items.`,
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        this.uomService.deleteUom(uom.Id).subscribe({
          next: () => {
            Swal.fire({
              title: 'Deleted!',
              text: 'The unit of measure has been removed.',
              icon: 'success',
              timer: 2000,
          ...
          error: (err) => {
            ...
            Swal.fire({
              title: 'Error!',
              text: 'Failed to delete. This unit might be in use by active items.',
              icon: 'error',
  ...
  applyFilter(event: Event) {
    ...
    this.filteredDataSource = this.dataSource.filter(uom => 
      uom.Name.toLowerCase().includes(filterValue) || 
      uom.Abbreviation.toLowerCase().includes(filterValue)
    );
  }
}
